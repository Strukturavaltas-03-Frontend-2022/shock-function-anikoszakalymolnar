...
const getSellPrice = function (price) {
  const bigPrice = Math.round(price * 1.35);
  return bigPrice;
};
...
const getActionPrice = function (price) {
  const smallPrice = Math.round(price * 1.35 * 0.9);
  return smallPrice;
};
...
const getProductDescrition = (product) => {
  const titleName = product.name[0].toUpperCase() + product.name.slice(1);
  const price = Math.round(product.price * 1.35);
  return `${titleName}, $${price}, in stock: ${product.stock}`;
};
 ...
 */
const getBio = (member) => {
  const titleName = member.name[0].toUpperCase() + member.name.slice(1);
  return `${titleName}, ${member.age} years old, live in: ${member.city}.`;
};
